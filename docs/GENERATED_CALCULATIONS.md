...
| `ch08-003` | 8 | PAE with real rate | If C=200+0.6Y-20r and I=80-10r, with no other sectors, what is PAE? | `PAE = C + I`; combine intercept, `Y`, and `r` terms, then evaluate a generated `Y` and `r` | Yes | The symbolic addition is instantiated with generated coefficients and values. The course convention treats displayed `r` values as percentage-point units in this equation. | `generated-pae-with-real-rate` |
| `ch08-005` | 8 | Derive AD | If equilibrium output is Y=700-75r and the PRF is r=1.5+0.5π, what is the AD curve? | Substitute `r = policy intercept + policy response × π` into `Y = intercept − coefficient × r`; evaluate at generated `π` | Yes | A single numeric field evaluates the same substitution while the worked steps preserve the AD derivation. | `generated-ad-substitution` |
...
| `ch10-026` | 10 | Growth accounting | If output grows 3.5%, K grows 2%, L grows 2%, and α=0.25, what is TFP growth? | `g_A = g_Y − αg_K − (1 − α)g_L` | Yes | Output, capital, labour growth, and alpha vary; residual TFP can be positive or negative. | `generated-tfp-growth` |
| `ch10-027` | 10 | Growth accounting | If α=0.3, K grows 5%, L grows 1% and TFP grows 2%, what is predicted output growth? | `g_Y = g_A + αg_K + (1 − α)g_L` | Yes | TFP, capital, labour growth, and alpha vary; weighted contributions are reported in percentage points. | `generated-output-growth-accounting` |
...
numeric evaluation of the same algebra so the primary interaction remains
numeric entry rather than string or equation grading.
...
| `ch08-003` | `280+0.6Y−30r` | Add both interest-rate coefficients and investment intercept. | `PAE`, `real-interest-rate`, `calculation` | Textbook Ch 8; Week7_Lecture2; Worksheet 7; Tutorial solutions week 9 |
| `ch08-005` | `Y=587.5−37.5π` | Substitute PRF into output; AD slope is negative. | `AD`, `derivation`, `calculation` | Textbook Ch 8; Week7_Lecture2; Worksheet 7; Tutorial solutions week 9 |
...
| `ch10-026` | `1.5%` | Weighted input contribution is subtracted from output growth. | `growth-accounting`, `calculation` | Textbook Ch 10; Week9_Lecture1; Tutorial solutions week 10 |
| `ch10-027` | `4.2%` | Add TFP and weighted input contributions as percentage points. | `growth-accounting`, `calculation` | Textbook Ch 10; Week9_Lecture1; Tutorial solutions week 10 |
...
session seed at the UI boundary. `New numbers` derives a fresh UI seed for the
same template and resets the instance timer.
...
then fills a smaller chapter pool with fresh variants. It avoids duplicate
prompt/stimulus fingerprints within one session. The generated mode offers All
chapters or Chapters 1–10 and set sizes 5, 10, or 20; no misleading Chapter 0
option is shown.
...
review. New numbers before submission creates no review and resets the timer.
...
prints distinct answers, distinct prompt/stimulus instances, minimum and
...
| Template | Distinct answers | Distinct instances | Min | Max | Positive / negative / zero |
| --- | ---: | ---: | ---: | ---: | ---: |
...
| `generated-pae-with-real-rate` | 72 | 100 | 320 | 1,210 | 100 / 0 / 0 |
| `generated-ad-substitution` | 88 | 100 | 132.5 | 840 | 100 / 0 / 0 |
...
| PAE and AD | Equation coefficients, Y/r or π | PAE values vary; AD output responds negatively to π | Numeric evaluation preserves the canonical algebra without showing the answer equation before submission. |
